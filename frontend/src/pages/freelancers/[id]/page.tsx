...
export default function FreelancerDetail({ userType = 'client' }: FreelancerDetailProps) {
  const { id } = useParams();
  const { token } = useAuth();
  const [activeTab, setActiveTab] = useState<'profile' | 'projects' | 'reviews'>('profile');
  const [freelancer, setFreelancer] = useState<FreelancerProfile | null>(null);
  const [participatedProjects, setParticipatedProjects] = useState<ProjectSummaryDto[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchFreelancerData = async () => {
      ...
      try {
        ...
        // 프리랜서 프로필 조회
        const { data: profileResponse } = await client.GET("/api/v1/members/{userId}/profile", {
          params: {
            path: {
              userId: Number(id)
            }
  ...
  useEffect(() => {
    const fetchEvaluations = async () => {
      ...
      try {
        ...
        const { data: evaluationsResponse } = await client.GET("/api/v1/evaluations/{id}", {
          params: {
            path: {
              id: Number(id)
            }
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-6">
              <div className="text-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-1">{freelancer.name || '프리랜서'}</h2>
                <p className="text-gray-600 mb-3">{freelancer.email || ''}</p>
                <div className="flex items-center justify-center mb-4">
                  <i className="ri-star-fill text-yellow-400 mr-1"></i>
                  <span className="font-semibold mr-2">{freelancer.ratingAvg?.toFixed(1) || '0.0'}</span>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {freelancer.skills && freelancer.skills.length > 0 ? (
                      ...
                      <span className="text-sm text-gray-400">등록된 기술이 없습니다</span>
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {freelancer.interests && freelancer.interests.length > 0 ? (
                      ...
                      <span className="text-sm text-gray-400">등록된 관심 분야가 없습니다</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              {userType === 'client' && (() => {
                console.log('🎨 버튼 렌더링:', { userType, isFavorite, favoriteLoading });
                return (
                  <div className="space-y-3">
                    ...
                    <Button
                      variant="outline"
                      className={`w-full transition-all duration-300 ${isFavorite
                    ...
                    >
                      <i className={`${isFavorite ? 'ri-heart-fill text-red-500' : 'ri-heart-line'
                        } mr-2 transition-all duration-300 ${favoriteLoading ? 'animate-pulse' : ''}`}></i>
                      {favoriteLoading ? '처리 중...' : isFavorite ? '관심 프리랜서 해제' : '관심 프리랜서 등록'}
                    </Button>
                  </div>
                );
              })()}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex">
                  {tabs.map((tab) => (
                    <button
                      ...
                      className={`flex-1 px-6 py-4 text-sm font-medium border-b-2 transition-colors cursor-pointer ${activeTab === tab.id
              ...
              <div className="p-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-gray-700 leading-relaxed">
                        {freelancer.comment || freelancer.introduction || '등록된 자기소개가 없습니다.'}
                      </p>
                    </div>
                    ...
                    {freelancer.job && (
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 mb-3">직무</h3>
                        <p className="text-gray-700">{freelancer.job}</p>
                      </div>
                    )}

                    {freelancer.career && Object.keys(freelancer.career).length > 0 && (
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 mb-3">기술 경력</h3>
                        <div className="grid grid-cols-2 gap-3">
                          {Object.entries(freelancer.career).map(([tech, months]) => (
                            <div key={tech} className="p-3 bg-gray-50 rounded-lg">
                              <p className="font-medium text-gray-900">{tech}</p>
                              <p className="text-sm text-gray-600">{months}개월</p>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2">
                            {participatedProjects.filter(p => p.status === 'IN_PROGRESS').length > 0 ? (
                              participatedProjects.filter(p => p.status === 'IN_PROGRESS').map((project) => (
                                <div key={project.id} className="p-3 bg-blue-50 rounded-lg">
                                  <p className="text-sm font-medium text-blue-900">{project.title}</p>
                                  <p className="text-xs text-blue-600 mt-1">{project.summary}</p>
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {participatedProjects.filter(p => p.status === 'COMPLETED').length > 0 ? (
                              participatedProjects.filter(p => p.status === 'COMPLETED').slice(0, 3).map((project) => (
                                <div key={project.id} className="p-3 bg-green-50 rounded-lg">
                                  <p className="text-sm font-medium text-green-900">{project.title}</p>
                                  <p className="text-xs text-green-600 mt-1">{project.summary}</p>
                                  <span className="text-xs text-green-600">완료</span>
                                </div>
                              ))
                            ) : (
                              <div className="p-3 bg-gray-50 rounded-lg">
                                <p className="text-sm text-gray-500">완료한 프로젝트가 없습니다</p>
                              </div>
                            )}
                          </div>
                        </div>
                ...
                {activeTab === 'projects' && (
                  <div className="space-y-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">참여 프로젝트</h3>
                    ...
                    {participatedProjects.length > 0 ? (
                        ...
                        {participatedProjects.map((project) => (
                          ...
                          >
                            <div className="flex items-start justify-between mb-3">
                              ...
                              <span className={`px-3 py-1 rounded-full text-xs font-medium ml-4 ${project.status === 'COMPLETED'
                      ...
                      <div className="text-center py-12">
                        ...
                        <h4 className="text-lg font-medium text-gray-900 mb-2">참여 프로젝트 없음</h4>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex items-center">
                        <i className="ri-star-fill text-yellow-400 mr-1"></i>
                        <span className="font-semibold mr-2">{freelancer.ratingAvg?.toFixed(1) || '0.0'}</span>
                        <span className="text-sm text-gray-500">(총 {evaluationCount}개)</span>
                      </div>
                    </div>
                        ...
                        {evaluations.map((evaluation) => {
                          const avgRating = Math.round(
                            ((evaluation.ratingSatisfaction || 0) +
                              ...
                              (evaluation.ratingProactiveness || 0)) / 5
                          );

                          return (
                            <div key={evaluation.evaluationId} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center">
                                  <div className="flex items-center">
                                    {[...Array(5)].map((_, i) => (
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                                <div className="text-center p-2 bg-white rounded">
                                  ...
                                  <p className="font-semibold text-gray-900">{evaluation.ratingSatisfaction || 0}</p>
                                </div>
                                <div className="text-center p-2 bg-white rounded">
                                  <p className="text-xs text-gray-600 mb-1">전문성</p>
                                  <p className="font-semibold text-gray-900">{evaluation.ratingProfessionalism || 0}</p>
                                </div>
                                <div className="text-center p-2 bg-white rounded">
                                  <p className="text-xs text-gray-600 mb-1">일정준수</p>
                                  <p className="font-semibold text-gray-900">{evaluation.ratingScheduleAdherence || 0}</p>
                                </div>
                                <div className="text-center p-2 bg-white rounded">
                                  <p className="text-xs text-gray-600 mb-1">소통능력</p>
                                  <p className="font-semibold text-gray-900">{evaluation.ratingCommunication || 0}</p>
                                </div>
                                <div className="text-center p-2 bg-white rounded">
                                  <p className="text-xs text-gray-600 mb-1">적극성</p>
                                  <p className="font-semibold text-gray-900">{evaluation.ratingProactiveness || 0}</p>
                                </div>
                              </div>
                            </div>
                          );

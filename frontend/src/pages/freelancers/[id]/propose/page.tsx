...
export default function FreelancerPropose() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data: projectsResponse } = await client.GET(
  ...
  const projectOptions = myProjects.map((project) => ({
    ...
    label: `${project.title || "제목 없음"} (예산: ${
      (project.price || project.budget)?.toLocaleString() || "미정"
    }원)`,
  }));
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  {(() => {
                    const selectedProject = myProjects.find(
                      (p) => p.id?.toString() === formData.projectId
                    );
                    return selectedProject ? (
                      <div className="text-sm text-blue-800">
                        <p className="font-medium">
                          {selectedProject.title || "제목 없음"}
                        </p>
                        <p className="mt-1">
                          {selectedProject.summary || "설명 없음"}
                        </p>
                        <div className="flex items-center space-x-4 mt-2">
                          <span>
                            예산:{" "}
                            {(
                              selectedProject.price || selectedProject.budget
                            )?.toLocaleString() || "미정"}
                            원
                          </span>
          ...
          <div className="flex">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="space-y-1 text-blue-700">
                ...
                <li>
                  • 프리랜서의 수락/거절 여부를 알림으로 받아보실 수 있습니다.
                </li>
